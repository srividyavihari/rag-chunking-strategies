...
There is no single best strategy. Start with the structure and behavior of the corpus:

| If your data looks like… | Start with… | Then test… |
|---|---|---|
| Clean Markdown, HTML, or manuals | Recursive by headings and paragraphs | Parent-child retrieval |
| Books, essays, or narrative text | Paragraph-based | Sentence grouping with neighbor expansion |
| Short FAQs or support answers | Sentence/grouped-sentence | Fixed-size baseline |
| Transcripts or chat logs | Speaker/turn-aware recursive chunks | Time-window overlap |
| Code | Language-aware functions/classes | Repository and symbol metadata |
| Dense research or legal material | Recursive structural chunks | Semantic boundaries and parent context |

### A sensible starting experiment

1. Build a fixed-token baseline.
2. Add a recursive splitter that respects headings and paragraphs.
3. Compare both with the same queries, embedding model, and retriever settings.
4. Tune chunk size and overlap using evidence, not intuition.
5. Add semantic chunking only if the quality gain justifies ingestion cost and complexity.

## Evaluate the strategy

Chunking should be measured at the retrieval and answer levels.

- **Retrieval recall@k:** Did the top *k* chunks contain the evidence?
- **Precision@k:** How many retrieved chunks were actually useful?
- **MRR or nDCG:** How highly was the best evidence ranked?
- **Context precision:** How much of the LLM context was relevant rather than noise?
- **Faithfulness:** Is the answer supported by retrieved evidence?
- **Operational cost:** How many chunks, embedding tokens, index bytes, and milliseconds did the strategy require?

Create a small evaluation set of representative questions with known supporting passages. Change one parameter at a time and inspect failures: missing context, split facts, duplicate hits, irrelevant neighbors, and oversized chunks.

## Practical production checklist

- Clean extraction artifacts before splitting, especially repeated headers, footers, and broken PDF lines.
- Count tokens with the tokenizer used by the target embedding or generation model.
- Keep stable chunk IDs and metadata so results are traceable to their source.
- Preserve heading paths and document relationships; avoid treating chunks as isolated blobs.
- Consider retrieving small chunks and expanding to their parent section for the LLM.
- Merge or deduplicate overlapping adjacent results before prompt construction.
- Version the chunker and re-index when its configuration changes.
- Benchmark with domain questions before declaring one strategy “best.”
